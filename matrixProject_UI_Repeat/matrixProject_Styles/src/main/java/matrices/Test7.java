...
public class Test7 implements Serializable
{
    ...
    public Test7() 
    {
        ...
        {
            MyString2[] numbers = new MyString2[this.columns];
            for (int j = 0; j < this.columns; j++) 
            {
                numbers[j] = new MyString2("");
            }
    ...
    {
        ArrayList<ArrayList<MyString2>> temp = new ArrayList<ArrayList<MyString2>>();
        ...
        for (int i = 0; i < this.rows; i++) 
        {
            temp.add(new ArrayList<MyString2>());
            for (int j = 0; j < this.columns; j++) 
            {
                temp.get(i).add(new MyString2(""));
                //temp.get(i).add(j, new MyString2(""));
            }
        }
...
//        this.valueOfDynamicInputTextFromConverter = "dummy value";
        System.out.println("listenForKeyUp " + row + ", " + column);
    ...
    {
        for (ArrayList<MyString2> row : this.matrix) 
        {
            for (MyString2 s : row) 
            {
