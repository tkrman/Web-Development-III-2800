...
public class Matrices implements Matrix
{
    ...
    static class MatricesSubtraction extends RecursiveTask<ArrayList<ArrayList<BigInteger>>>
    {
        ...
        ArrayList<ArrayList<BigInteger>> A;
        ArrayList<ArrayList<BigInteger>> B;
        ArrayList<ArrayList<BigInteger>> AminusB;
        final int HOW_MANY_ROWS_IN_PARALLEL = 3;//threshold
        int startIndex;
        int endIndex;
                ...
                ArrayList<ArrayList<BigInteger>> B)
        {
            ...
            this.A = A;
            this.B = B;
            AminusB = new ArrayList<ArrayList<BigInteger>>();
            ...
            if (this.endIndex - this.startIndex < HOW_MANY_ROWS_IN_PARALLEL)
              {
                ...
                for (int i = this.startIndex; i <= this.endIndex; ++i)
                  {
                    //>create a new row to add it to the resulting matrix resultC
                    ArrayList<BigInteger> rowAminusB = new ArrayList<BigInteger>();
                    ...
                    for (int j = 0; j < A.get(0).size(); j++)
                      ...
                      //and add all the Jth entries from each row
                      {
                        rowAminusB.add(A.get(i).get(j).subtract(B.get(i).get(j)));
                      }
                    resultC.add(rowAminusB);
                  }
                return resultC;
              }
            else
              {      //>> keep on FORKING the matrix until the 
                ...
                //>> join what the FORKs returned from the IFs 
                AminusB.addAll(firstHalf.join());
                AminusB.addAll(secondHalf.join());
